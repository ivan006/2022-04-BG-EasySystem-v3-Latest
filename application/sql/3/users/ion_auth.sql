...
CREATE TABLE `users_groups` (
  ...
  PRIMARY KEY (`id`),
  KEY `fkusers_groups_users1_idx` (`user_id`),
  KEY `fkusers_groups_groups1_idx` (`group_id`),
  CONSTRAINT `ucusers_groups` UNIQUE (`user_id`, `group_id`),
  CONSTRAINT `fkusers_groups_users1` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE NO ACTION,
  CONSTRAINT `fkusers_groups_groups1` FOREIGN KEY (`group_id`) REFERENCES `groups` (`id`) ON DELETE CASCADE ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
